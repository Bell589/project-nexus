  ...
  if (session) {
    ...
    return (
      <section>
        ...
        {session.ritterFusionMode&&<div style={{padding:"8px 10px",border:"1px solid #8b6",borderRadius:8,marginBottom:10}}><b>{session.ritterFusionMode==="full"?"Vollverschmelzung":"Teilverschmelzung"} aktiv</b> · Magier und Ritter sind ein Combatant.</div>}{session.ritterMaxHp&&session.ritterSummoned? <><HpBar label="Spektralritter" hp={session.ritterHp??0} maxHp={session.ritterMaxHp}/><ResourceBar label="Ritterenergie" value={session.ritterEnergy??0} max={session.ritterEnergyMax??1}/></>:null}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          {session.ritterSummoned&&<><button disabled={busy} onClick={()=>act("ritter_angriff")}>Spektralritter: eigener Angriff</button><button disabled={busy} onClick={()=>act("ritter_teilfusion")}>Teilverschmelzung</button><button disabled={busy} onClick={()=>act("ritter_vollfusion")}>Vollverschmelzung</button>{character.spektralritterPact?.individualAbilities.length?<select disabled={busy} defaultValue="" onChange={e=>{if(e.target.value)act("ritter_technik",e.target.value);e.target.value=""}}><option value="" disabled>Rittertechnik wählen...</option>{character.spektralritterPact.individualAbilities.map(a=><option key={a.name} value={a.name}>{a.name}</option>)}</select>:null}</>}
