    ...
    return (
      <section>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          {learnedJutsu.length>0&&<select disabled={busy} defaultValue="" onChange={e=>{if(e.target.value)act("jutsu",e.target.value);e.target.value=""}}><option value="" disabled>Jutsu wählen...</option>{learnedJutsu.map(st=>{const known=({bunshin:["Bunshin no Jutsu",8],kawarimi:["Kawarimi no Jutsu",10],henge:["Henge no Jutsu",6],shurikenjutsu:["Shurikenjutsu",4],goukakyuu:["Katon: Gōkakyū no Jutsu",18],housenka:["Katon: Hōsenka",22],"genjutsu-kasumi":["Kasumi-Genjutsu",16],rasengan:["Rasengan",28],chidori:["Chidori",30],"sharingan-genjutsu":["Sharingan: Genjutsu",20],amaterasu:["Amaterasu",45]} as Record<string,[string,number]>)[st.techniqueId];if(!known)return null;const locked=st.techniqueId.startsWith("sharingan")&&!session.activeDojutsu;return <option key={st.techniqueId} value={st.techniqueId} disabled={locked||session.characterResource<known[1]}>{known[0]} ({known[1]} Chakra){locked?" — benötigt aktives Sharingan":""}</option>})}</select>}
