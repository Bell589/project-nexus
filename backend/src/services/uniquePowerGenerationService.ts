 ...
 */
export function generateUniquePower(origin: UniquePowerOrigin, preferredVariant?: string): UniquePowerInstance {
  const variant =
    preferredVariant && origin.variantPool.includes(preferredVariant)
      ? preferredVariant
      : origin.variantPool[Math.floor(Math.random() * origin.variantPool.length)];
